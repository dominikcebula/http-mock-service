...
```shell
docker run --rm -p 8080:8080 dominikcebula/http-mock-service:v0.4.2
```
...
```shell
docker run --mount type=bind,source=./my_config.yaml,target=/app/config.yaml --rm -p 8080:8080 dominikcebula/http-mock-service:v0.4.2
```
